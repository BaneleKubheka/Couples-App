...
function show(id){ $$('.screen').forEach(x=>x.classList.remove('active')); const target=$('#'+id); if(target) target.classList.add('active'); else console.warn('Missing screen:', id); }
...
function bindUI(){
  ...
  $$('.tab').forEach(b=>b.onclick=()=>{ $$('.tab,.tab-panel').forEach(x=>x.classList.remove('active')); b.classList.add('active'); const panel=$('#'+b.dataset.tab); if(panel) panel.classList.add('active'); else console.warn('Missing tab panel:', b.dataset.tab); renderAll(); });
...
async function saveMood(){ const el=$('#moodNote'); await dbInsert('moods',{id:uid(),profile_id:APP.profile.id,mood:APP.selectedMood,note:el?.value||'',created_at:now()}); if(el) el.value=''; await loadAll(); renderAll(); }
async function saveNote(){ const el=$('#publicNote'); const body=(el?.value||'').trim(); if(!body)return; await dbInsert('notes',{id:uid(),profile_id:APP.profile.id,body,created_at:now()}); if(el) el.value=''; await loadAll(); renderAll(); }
...
async function createAlbum(){
  const albumNameEl=$('#albumName'), albumStatus=$('#albumStatus'), albumBtn=$('#createAlbumBtn'); const name=(albumNameEl?.value||'').trim(); if(!name){ if(albumStatus) albumStatus.textContent='Enter an album name.'; return; }
  if(albumBtn) albumBtn.disabled=true; if(albumStatus) albumStatus.textContent='Creating album...';
  try{ await dbInsert('albums',{id:uid(),owner_id:APP.profile.id,name,visibility:$('#albumVisibility')?.value||'private',created_at:now()}); if(albumNameEl) albumNameEl.value=''; await loadAll(); renderAll(); if(albumStatus) albumStatus.textContent='Album created.'; }
  catch(e){ console.error(e); if(albumStatus) albumStatus.textContent='Album creation failed: '+friendlySupabaseError(e); }
  finally{ if(albumBtn) albumBtn.disabled=false; }
}
...
async function uploadMedia(){
  ...
  const uploadBtn=$('#uploadMediaBtn'); if(uploadBtn) uploadBtn.disabled=true; setMediaStatus(`Preparing ${files.length} upload(s). Images will be compressed automatically; videos over 500MB will be compressed where the browser supports it.`);
  ...
  finally{ if(uploadBtn) uploadBtn.disabled=false; await loadAll(); renderAll(); }
}
...
async function sendEncryptedMessage(){ const to=$('#messageTo')?.value||'', body=($('#messageBody')?.value||'').trim(), phrase=$('#messagePhrase')?.value||''; if(!to||!body||!phrase)return toast('Choose recipient, message and shared phrase.'); const enc=await encryptText(body,phrase); await dbInsert('messages',{id:uid(),from_id:APP.profile.id,to_id:to,cipher:enc.cipher,iv:enc.iv,created_at:now()}); const mb=$('#messageBody'); if(mb) mb.value=''; await loadAll(); renderAll(); }
...
function renderGalleryModal(){
  ...
  setText('#galleryTitle', p.name||'Media');
  setText('#galleryCounter', `${APP.galleryIndex+1} / ${APP.galleryItems.length}`);
...
async function saveCallNote(){ const el=$('#callNoteBody'); const body=(el?.value||'').trim(); if(!body) return toast('Write a call note first.'); await dbInsert('activities',{id:uid(),profile_id:APP.profile.id,body:'Call note: '+body,created_at:now()}); if(el) el.value=''; await loadAll(); renderAll(); }
...
// In-app WebRTC calling was replaced by the external Call Hub.
// These no-op functions are kept only to avoid crashes if an old cached button calls them.
async function getMedia(){ toast('In-app calling has been replaced by Call Hub.'); }
function newPeer(){ return null; }
async function startCall(){ toast('Use the Call Hub buttons for WhatsApp, phone, or FaceTime.'); }
async function joinLatestCall(){ toast('Use the Call Hub buttons for WhatsApp, phone, or FaceTime.'); }
function listenSignals(){}
async function handleSignal(){}
async function setCallRecording(){}
function bestRecordingMime(){ return ''; }
function mixedStream(){ return new MediaStream(); }
...
function setupInstallPrompt(){
  ...
  const banner=$('#installBanner'), install=$('#installBtn'), dismiss=$('#dismissInstall');
  const standalone=matchMedia('(display-mode: standalone)').matches || navigator.standalone;
  if(standalone || !banner || !install || !dismiss) return;
  window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();promptEvent=e;banner.classList.remove('hidden')});
  install.onclick=async()=>{ if(promptEvent){promptEvent.prompt(); await promptEvent.userChoice; banner.classList.add('hidden');} else toast('On iPhone/iPad: tap Share, then Add to Home Screen. On Android: use the browser menu, then Install/Add to Home screen.'); };
...
Object.assign(window,{createProfile,loginProfile,loadProfile,createAlbum,uploadMedia,shareLocationOnce,startLiveLocation,stopLiveLocation,openGallery,closeGallery,galleryNext,galleryPrev,sendLinkRequest,acceptLinkRequest,rejectLinkRequest,removeLinkedProfile,moveMediaFromGallery,deleteMediaFromGallery,renameAlbum,deleteAlbum});
