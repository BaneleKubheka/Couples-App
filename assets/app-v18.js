console.log('Couples Connect app version: callhub-profile-fix-20260617-18');
...
function renderAll(){ if(!APP.profile)return; $('#activeProfileText').textContent=`Signed in as ${APP.profile.name}`; $('#profilePreview').textContent=JSON.stringify({profileCode:APP.profile.id, name:APP.profile.name},null,2); fillProfileEdit(); renderFeeds(); renderProfiles(); renderAlbums(); renderSelectors(); renderMessages(); renderRecordingBanner(); renderLocations(); renderSettingsLinking(); renderLinkRequests(); renderCallHub(); renderCallNotes(); applyAdminVisibility(); }
function fillProfileEdit(){ $('#editName').value=APP.profile.name||''; $('#editBasics').value=APP.profile.basics||''; $('#editPersonality').value=APP.profile.personality||''; $('#editNeeds').value=APP.profile.needs||''; $('#editLife').value=APP.profile.life||''; if($('#editPhone')) $('#editPhone').value=APP.profile.phone||''; if($('#editWhatsApp')) $('#editWhatsApp').value=APP.profile.whatsapp||''; if($('#editFacetime')) $('#editFacetime').value=APP.profile.facetime||''; }
...
function renderProfiles(){ const ids=visibleProfileIds(); $('#publicProfiles').innerHTML=APP.profiles.filter(p=>ids.includes(p.id)).map(p=>`<div class="partner-card"><h3>${escapeHtml(p.name)}</h3><p><b>Basics:</b> ${escapeHtml(p.basics||'')}</p><p><b>Personality:</b> ${escapeHtml(p.personality||'')}</p><p><b>Needs:</b> ${escapeHtml(p.needs||'')}</p><p><b>Life:</b> ${escapeHtml(p.life||'')}</p><p><b>Phone:</b> ${escapeHtml(p.phone||'Not supplied')}</p><p><b>WhatsApp:</b> ${escapeHtml(p.whatsapp||p.phone||'Not supplied')}</p><p><b>FaceTime:</b> ${escapeHtml(p.facetime||p.phone||'Not supplied')}</p></div>`).join(''); }
